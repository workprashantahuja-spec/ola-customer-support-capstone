...
`governance.py` applies the course's four governance layers:
...
Before CrewAI runs, `RuntimeBudget` estimates input tokens plus a conservative reserve for the full pipeline. The default limits are 256 input tokens, 1,024 estimated total tokens, and USD 0.001 reference cost. The submitted mock path actually bills USD 0; the reference rate is deliberately retained so the same control remains meaningful if a paid model is introduced later.
